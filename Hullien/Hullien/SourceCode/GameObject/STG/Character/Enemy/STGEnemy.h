...
namespace STG
{
	class CEnemy : public STG::CCharacter
	{
		...
		const int	BULLET_COLL_DISAPPEAR	= 1;		// 弾が衝突した際に弾を消すか.
		...
		const int	SHOT_BULLET_COUNT		= 3;		// 一発で出る弾の数.
		const float SHOT_ANGLE				= static_cast<float>(D3DXToRadian(10.0));	// 撃つ時の角度.
		const int	SHOT_NUMBER				= 1;	// 弾の番号 1 : 一発ずつ, 2 : 複数.
