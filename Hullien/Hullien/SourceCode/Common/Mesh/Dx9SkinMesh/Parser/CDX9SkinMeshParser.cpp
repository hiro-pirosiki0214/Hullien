...
bool D3DXPARSER::SetMatrixFromBone(const char* sBoneName, D3DXMATRIX inMat)
{
	LPD3DXFRAME pFrame;
	pFrame = (MYFRAME*)D3DXFrameFind(m_pFrameRoot, sBoneName);
	...
	//位置情報を取得.
	pFrame->TransformationMatrix = inMat;

	return true;
}

//指定したボーン情報(行列)を設定する関数.
bool D3DXPARSER::SetPosFromBone(const char* sBoneName, D3DXVECTOR3 inPos)
{
	LPD3DXFRAME pFrame;
	pFrame = (MYFRAME*)D3DXFrameFind(m_pFrameRoot, sBoneName);
	...
	D3DXMATRIX mBone = pFrame->TransformationMatrix;
	mBone._41 = inPos.x;
	mBone._42 = inPos.y;
	mBone._43 = inPos.z;

	if (SetMatrixFromBone(sBoneName, mBone) == false) {
		return false;
	}
