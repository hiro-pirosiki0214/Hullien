...
CTranslucentShader* CTranslucentShader::GetInstance()
{
	static std::unique_ptr<CTranslucentShader> pInstance = 
		std::make_unique<CTranslucentShader>();
	return pInstance.get();
}

// 初期化.
HRESULT CTranslucentShader::Init()
{
...
// 解放.
HRESULT CTranslucentShader::ReleaseShader()
{
	GetInstance()->Release();
	return S_OK;
}

void CTranslucentShader::SetConstantBufferData( const D3DXMATRIX& mWVP, const bool isSkin )
{
...
// 各種シェーダの設定.
//-------------------------------------.
void CTranslucentShader::ShaderSet( ID3D11Buffer* pVertexBuffer, ID3D11Buffer* pIndexBuffer )
{
	// 使用するシェーダのセット.
	GetInstance()->m_pContext11->VSSetShader( GetInstance()->m_pVertexShader, nullptr, 0 );	// 頂点シェーダ.
	GetInstance()->m_pContext11->PSSetShader( GetInstance()->m_pPixelShader, nullptr, 0 );	// ピクセルシェーダ.
...
// 初期化.
HRESULT CTranslucentShader::Init( ID3D11Device* pDevice11, ID3D11DeviceContext* pContext11 )
{
...
// 解放.
HRESULT CTranslucentShader::Release()
{
...
// シェーダー作成.
HRESULT CTranslucentShader::InitShader()
{
